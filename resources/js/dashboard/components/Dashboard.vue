  ...
  <div>
    ...
    <div :class="['fixed inset-y-0 left-0 z-30 bg-gray-800 text-white transition-all duration-300 ease-in-out flex flex-col h-full', isSidebarOpen ? 'w-64' : 'w-20']">
      <div class="px-8 py-6 text-center overflow-hidden">
        <h2 class="text-2xl font-semibold whitespace-nowrap transition-all duration-300" :class="isSidebarOpen ? 'max-w-xs' : 'opacity-0 max-w-0'">Dashboard</h2>
      </div>
      <nav class="flex-grow px-4 py-2 space-y-2">
        <a href="#" @click.prevent="currentDashboardView = 'billing'"
          ...
          <svg :class="['w-6 h-6 transition-all duration-300', isSidebarOpen ? 'mr-3' : 'mx-auto']" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
          <span class="whitespace-nowrap transition-all duration-300 overflow-hidden" :class="isSidebarOpen ? 'max-w-xs' : 'opacity-0 max-w-0'">Facturación Masiva</span>
        </a>
        <a href="#" @click.prevent="currentDashboardView = 'status'"
          ...
          <span class="whitespace-nowrap transition-all duration-300 overflow-hidden" :class="isSidebarOpen ? 'max-w-xs' : 'opacity-0 max-w-0'">Estado de factura</span>
        </a>
        <a href="#" @click.prevent="currentDashboardView = 'my-invoices'"
          ...
          <span class="whitespace-nowrap transition-all duration-300 overflow-hidden" :class="isSidebarOpen ? 'max-w-xs' : 'opacity-0 max-w-0'">Mis Comprobantes</span>
        </a>
        <a href="#" @click.prevent="currentDashboardView = 'corrective'"
            ...
            <span class="whitespace-nowrap transition-all duration-300 overflow-hidden" :class="isSidebarOpen ? 'max-w-xs' : 'opacity-0 max-w-0'">Facturación Correctiva</span>
        </a>
        <a href="#" @click.prevent="currentDashboardView = 'configuration'"
            ...
            <span class="whitespace-nowrap transition-all duration-300 overflow-hidden" :class="isSidebarOpen ? 'max-w-xs' : 'opacity-0 max-w-0'">Configuración</span>
        </a>
      </nav>
    ...
    async processSingleInvoice(row) {
      try {
        ...
        if (error.message === 'Cédula no válida') {
          ...
          this.addFailedRowToCorrective(row, 'Teléfono debe tener 10 dígitos (o 9 para corrección).');
