<template>
    <div class="bg-white rounded-xl shadow-lg">
        <div class="flex justify-between items-center mb-4 px-6 pt-6">
            <h3 class="text-xl font-bold text-gray-800">Gestionar Retenciones</h3>
            <div class="flex space-x-4">
                <RefreshButton :is-loading="isLoading" @click="fetchWithholdings" />
        ...
        <DataTable v-else :data="processedWithholdings" :headers="headers" :sort-key="sortKey" :sort-order="sortOrder" @sort="sortBy">
...
import WithholdingVoucherModal from './WithholdingVoucherModal.vue';
import RefreshButton from './RefreshButton.vue';
import TableSkeleton from './TableSkeleton.vue';

export default {
    name: 'WithholdingsManager',
    components: {
        DataTable,
        BaseButton,
        WithholdingVoucherModal,
    ...
    computed: {
        processedWithholdings() {
            let filtered = [...this.withholdings];
    ...
    methods: {
        ...
        async fetchWithholdings() {
            this.isLoading = true;
            try {
                const response = await axios.get('/api/withholding-vouchers', {
                    headers: { 'Authorization': `Bearer ${this.token}` }
                });
                this.withholdings = response.data.data.data;
            } catch (error) {
        ...
        openCreateModal() {
            this.selectedWithholding = null;
            this.isModalOpen = true;
        },
        openEditModal(withholding) {
            this.selectedWithholding = withholding;
            this.isModalOpen = true;
        },
        closeModal() {
        ...
        async handleSave(withholdingData) {
            if (this.selectedWithholding) {
                await this.handleUpdate(withholdingData);
            } else {
                await this.handleCreate(withholdingData);
            }
        },
        async handleCreate(data) {
            this.isSubmitting = true;
            try {
                await axios.post('/api/withholding-vouchers', data, {
                    headers: { 'Authorization': `Bearer ${this.token}` }
                });
                this.closeModal();
                await this.fetchWithholdings();
        ...
        async handleUpdate(data) {
            this.isSubmitting = true;
            try {
                await axios.put(`/api/withholding-vouchers/${data.id}`, data, {
                    headers: { 'Authorization': `Bearer ${this.token}` }
                });
                this.closeModal();
                await this.fetchWithholdings();
        ...
        async confirmDelete(withholding) {
            if (window.confirm(`¿Está seguro de que desea eliminar la retención "${withholding.secuencial}"?`)) {
                this.isLoading = true;
                try {
                    await axios.delete(`/api/withholding-vouchers/${withholding.id}`, {
                        headers: { 'Authorization': `Bearer ${this.token}` }
                    });
                    await this.fetchWithholdings();
                } catch (error) {
